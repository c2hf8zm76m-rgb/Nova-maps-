(function(){
  'use strict';
  var CACHE='audify_lyrics_cache_v38', TIMEOUT=4400;
  var st={id:'',track:null,data:null,promise:null,open:false,button:null,panel:null,scroll:null,meta:null,lines:[],active:-2,timer:0};
  ...
  function ensure(){
    ...
    if(!panel){panel=document.createElement('section');panel.id='v38Karaoke';panel.className='v38-karaoke';panel.innerHTML='<div class="v38-head"><div><b>Paroles</b><span id="v38Meta">Audify Karaoke</span></div><button class="v38-close" type="button" aria-label="Fermer">×</button></div><div id="v38Scroll" class="v38-scroll"></div>';view.appendChild(panel);panel.querySelector('.v38-close').addEventListener('click',closePanel)}
    st.panel=panel;st.scroll=document.getElementById('v38Scroll');st.meta=document.getElementById('v38Meta');return true;
  }
  ...
  function parseLrc(src,duration){var out=[],rows=String(src||'').split(/\r?\n/);for(var r=0;r<rows.length;r++){var raw=rows[r],re=/\[(\d{1,2}):(\d{2}(?:\.\d{1,3})?)\]/g,times=[],m;while((m=re.exec(raw)))times.push(Number(m[1])*60+Number(m[2]));if(!times.length)continue;var t=txt(raw.replace(re,''));if(!t)continue;for(var j=0;j<times.length;j++)out.push({start:times[j],text:t})}out.sort(function(a,b){return a.start-b.start});for(var i=0;i<out.length;i++){out[i].end=out[i+1]?out[i+1].start:(duration||out[i].start+5);if(out[i].end<=out[i].start)out[i].end=out[i].start+2.5}return out}
  function render(data){
    if(!ensure())return;st.scroll.innerHTML='';st.lines=[];st.active=-2;st.meta.textContent=txt((data.trackName||st.track.title||'')+' • '+(data.artistName||st.track.artist||''));
    if(data.syncedLyrics){var lines=parseLrc(data.syncedLyrics,data.duration||pdur());if(!lines.length){status('Synchronisation indisponible','Les paroles ont été trouvées mais sans repères temporels utilisables.');return}var f=document.createDocumentFragment();for(var i=0;i<lines.length;i++)(function(line){var el=document.createElement('div');el.className='v38-line';el.textContent=line.text;el.addEventListener('click',function(){try{if(typeof S!=='undefined'&&S.ready&&S.p)S.p.seekTo(line.start,true)}catch(e){}});f.appendChild(el);st.lines.push({el:el,line:line})})(lines[i]);st.scroll.appendChild(f);startTimer()}
  ...
  function update(t){var idx=-1;for(var i=0;i<st.lines.length;i++){if(t>=st.lines[i].line.start)idx=i;else break}if(idx===st.active)return;st.active=idx;for(var j=0;j<st.lines.length;j++){st.lines[j].el.classList.toggle('active',j===idx);st.lines[j].el.classList.toggle('past',j<idx);st.lines[j].el.classList.toggle('next',j===idx+1)}if(idx>=0)try{st.lines[idx].el.scrollIntoView({block:'center',behavior:'smooth'})}catch(e){}}
