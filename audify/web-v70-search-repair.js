...
async function altSearch(q){
  ...
  const piped=await firstSuccessful(pipedHosts.map(h=>async()=>{const j=await fetchTimeout(`${h}/search?q=${enc}&filter=videos`,6500);return(j?.items||[]).map(normalizePiped).filter(Boolean).slice(0,24)}));
  return piped;
}
...
function correctStoredMeta(t){
  let h=read(KEY.hist);const hi=h.findIndex(x=>x.id===t.id);if(hi>=0){h[hi]={...h[hi],...t,at:h[hi].at||Date.now()}}else h.unshift({...t,at:Date.now()});write(KEY.hist,h.slice(0,100));
  for(const k of [KEY.fav,KEY.queue]){let a=read(k),i=a.findIndex(x=>x.id===t.id);if(i>=0){a[i]={...a[i],...t};write(k,a)}}
}
function applyMeta(t){
  activeMeta=t;
  const pairs=[['#bigTitle',t.title],['#bigArtist',t.artist],['#pn',t.title],['#pa',t.artist]];for(const [s,v] of pairs){const e=$(s);if(e)e.textContent=v}
  for(const s of ['#bigCover','#pt']){const e=$(s);if(e)e.src=t.thumbnail}
  const vinyl=$('#vinyl');if(vinyl)vinyl.style.setProperty('--vinyl-label',`url("${t.thumbnail.replace(/"/g,'')}")`);
  correctStoredMeta(t);
  try{if('mediaSession'in navigator&&typeof MediaMetadata!=='undefined')navigator.mediaSession.metadata=new MediaMetadata({title:t.title,artist:t.artist,album:'Audify • YouTube',artwork:[{src:t.thumbnail}]})}catch{}
  window.dispatchEvent(new CustomEvent('audify:track-metadata',{detail:t}));
}
async function openTrack(t,i){
  if(!t)return;repairIndex=i;activeMeta=t;const qBefore=lastQuery||input.value.trim();
  input.value=t.id;
  try{await legacySearch?.call(go)}catch{}
  let tries=0;const timer=setInterval(()=>{tries++;const b=$('[data-play="0"]')||$('[data-open="0"]');if(b){clearInterval(timer);b.click();setTimeout(()=>{applyMeta(t);input.value=qBefore;renderResults(repairItems)},70)}else if(tries>35){clearInterval(timer);input.value=qBefore;say('Impossible d’ouvrir ce résultat YouTube')}},40)
}
async function repairedSearch(){
  ...
  if(direct){return legacySearch?.call(go)}
  ...
  try{const arr=await altSearch(q);if(seq!==searchSeq)return;repairItems=arr;repairIndex=-1;if(!arr.length)throw Error('Aucun résultat');renderResults(arr)}catch(e){if(seq!==searchSeq)return;content.className='empty';content.innerHTML='<b>Recherche YouTube momentanément indisponible</b><br><br>Audify a essayé plusieurs moteurs de secours.';say('Recherche YouTube indisponible')}
}
...
window.AudifySearchRepair={search:repairedSearch,getResults:()=>repairItems.slice(),getActive:()=>activeMeta};
})();